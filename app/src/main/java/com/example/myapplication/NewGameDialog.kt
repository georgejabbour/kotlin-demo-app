...
fun NewGameDialog(onStartGame: (Int) -> Unit, onDismiss: () -> Unit) {
    ...
    fun setSelectedOptionAndStartGame(selectedOptionParam: Int) {
        // Start the game with the selected starting life total
    ...
    Dialog(onDismissRequest = onDismiss) {
        Surface {
            Column(
                ...
                Row {
                    Button(onClick = { setSelectedOptionAndStartGame(20) }) {
                        Text(text = "20")
                    }
                    Spacer(modifier = Modifier.width(16.dp))
                    Button(onClick = { setSelectedOptionAndStartGame(40) }) {
                        Text(text = "40")
                    }
                }
            }
